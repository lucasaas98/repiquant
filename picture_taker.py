...
def get_extremes_timestamps_from_file(ticker, interval):
    all_files = sorted(h.get_ticker_files(ticker, interval))
    try:
        return max([int(file.split("_")[1].split(".")[0]) for file in all_files]), min(
            [int(file.split("_")[0]) for file in all_files]
        )
    except Exception as e:
        print(e)
    ...
    for interval in h.get_intervals():
        ...
        for ticker in data_api.get_actionable_stocks_list():
            ...
            if not os.path.exists(f"raw_data/{ticker}/{interval}"):
                print("\tFirst run! Creating folder!")
                os.makedirs(f"raw_data/{ticker}/{interval}")
            if os.path.exists(f"raw_data/{ticker}/{interval}/fucked_in_the_api"):
                flag = False

            while flag:
                max_timestamp, min_timestamp = get_extremes_timestamps_from_file(ticker, interval)
                try:
                    if min_timestamp > (time.time() - 86400 * 1095) and not os.path.exists(
                        f"raw_data/{ticker}/{interval}/less_than_2_years"
                    ):
                        if min_timestamp == 999999999999999:
                            dataframe = get_bar_data(ticker=ticker, interval=interval)
                        else:
                            end_date = datetime.datetime.fromtimestamp(min_timestamp).strftime("%Y-%m-%d %H:%M:%S")
                            dataframe = get_bar_data(ticker=ticker, interval=interval, end_date=end_date)
                        min_timestamp_df, max_timestamp_df = get_extremes_timestamps(dataframe)
                        min_timestamp_date = datetime.datetime.fromtimestamp(min_timestamp_df).strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        max_timestamp_date = datetime.datetime.fromtimestamp(max_timestamp_df).strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
